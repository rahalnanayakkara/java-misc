...
public class MemoryMaze extends MazeCanvas 
{
	...
	{
		WriteFile.open("Data2.dat");
		for(int x=0; x<14; x++) for(int y=0; y<14; y++)
		{
			WriteFile.writeInt(cell[x][y].wall[NORTH]?1:0);
			WriteFile.writeInt(cell[x][y].wall[SOUTH]?1:0);
			WriteFile.writeInt(cell[x][y].wall[EAST]?1:0);
			WriteFile.writeInt(cell[x][y].wall[WEST]?1:0);
		}
		WriteFile.close();
	}
